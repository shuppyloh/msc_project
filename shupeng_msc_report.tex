...
In this section, we reason about isolation and cooperation using our formal definition of permission and authority. Before we define isolation and cooperation, we first propose a lemma on the relationship between permission and authority:\\

\begin{logic}
\hr\\
\emp{Lemma---[Permission(Indirect,Eventual) is necessary for Authority]}\\
...
Given lemma \textsc{epc}, we can enforce isolation of o$'$ from o given the present path configuration of a system at \losigma, and when the closed system is eventually opened such that an unknown object has permission and authority of \textit{only} object o in some arising state \losigma', we know that the unknown code will not have permission of o$'$, since o is isolated from o$'$.\\

From the contraposition of the lemma [Permission is necessary for Authority], we have the corollary:\\
...
Therefore, it is good to know we can guarantee the internal state and behaviour of an object o$'$ in the presence of unknown code on object o through isolation, but in practice this is not entirely useful. The quintessential question in OCap systems is whether we can allow cooperation without vulnerability. What then is cooperation?\\

We say \textit{directed} cooperation from o to o$'$ is simply that o has permission of o$'$:\\
...
However, if we have another object o* that is of unknown provenance where o* belongs to a class UnknownObj which we do not have code knowledge of, then:\\

\begin{logic}
\hr\\
\emp{Definition---[Vulnerable cooperation]}\\
M,\losigma\ \loturns\ VulnerableCooperation(o*,o$'$) \loiff\ o*:UnknownObj \loand\ Cooperation(o*,o$'$)\\
\hr
\end{logic}
...
M,\losigma\ \loturns\ ProtectedCooperation(o*,o$'$) \loiff\ \=o*:UnknownObj \loand\ Cooperation(o*,o$'$) \loand\ \\
...
\begin{logic}
\hr\\
\emp{Lemma---[Protected cooperation implies Safe cooperation]}\\
M,\losigma\ \loturns\ \=ProtectedCooperation(o*,o$'$) \loand\ Dom(S,o$'$) \loand\ \loforall s. [ s\loin S \loimplies s:KnownObj ]\\
\>\loimplies\\
\> \loexists s$'$:KnownObj. [ VulnerableCooperation(o*,s$'$) \loand\ SafeCooperation(s$'$,o$'$) ]\\
\hr
\end{logic}

This says that if we have a configuration where object o$'$ is dominated by known objects, then the permission to use o$'$ by o* must involve at least a known object s$'$. This shows that we can use attenuating objects to mitigate vulnerability between unknown code and known code in OCap systems.\\
...
Protection strategies for attenuating objects can be implemented using defensive programming techniques like encapsulation and checking of conditions on method arguments. At the extreme end, attenuating objects can take the shape of either deciding to forward messages from the unknown object o* in a totally intact way, thereby allowing MayCall(o*,o$'$) to hold; or if some condition fails, the attenuating object can choose not to forward the message at all such that MayCall(o*,o$'$) does not hold. Somewhere in the middle, attenuating objects can also choose to modify the message from o* such that it becomes safe to forward, where MayCall(o*,o$'$) holds, but since it has been attenuated by the trusted attenuating object, we would know that the message from the attenuating object would be safe. What and how the attenuating objects decide to do with messages from unknown objects before forwarding them can be best expressed as OCap policies for the attenuating object.\\
...
\clearpage
\subsubsection{Policies for Caretaker Pattern}\label{sec:caretakerpolicies}
...
\end{minipage}
\clearpage
\subsubsection{Policies for Membrane Pattern}\label{sec:membranepolicies}
Here we show the important policies for the Membrane pattern. Unlike the Caretaker pattern, a Membrane does not leak capabilities because the membrane creates deep attenuating new membranes to mask capabilities returned by its protected object, and capabilities in messages passed by its caller.\\

Every membrane and lock object has a ghost field \sf{parent}: \begin{itemize}
